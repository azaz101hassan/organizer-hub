...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const storedState = req.cookies.get("oh_admin_state")?.value;
  const verifier = req.cookies.get("oh_admin_pkce")?.value;
  ...
  res.cookies.set("oh_admin_session", tokens.id_token, {
    httpOnly: true,
    path: "/",
    sameSite: "lax",
    maxAge: tokens.expires_in ?? 3600,
  });
  res.cookies.set("oh_admin_access_token", tokens.access_token, {
    httpOnly: true,
    path: "/",
    sameSite: "lax",
    maxAge: tokens.expires_in ?? 3600,
  ...
  if (tokens.refresh_token) {
    res.cookies.set("oh_admin_refresh", tokens.refresh_token, {
      httpOnly: true,
      path: "/",
      sameSite: "lax",
      maxAge: 60 * 60 * 24 * 14,
